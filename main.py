...
from visualize_f import visualize_trajectory, visualize_heatmap, save_figure_to_html, visualize_heatmap_with_centers
from kmeans import get_addr

floor_data_dir = './data/site1/F2'
...
def save_way_data(value):
    way = [len(i) for i in value]
    data = {
        '均值': np.mean(way),
        '方差': np.var(way),
        '标准差': np.std(way),
    ...
    df = pd.DataFrame(list(data.items()))
    df.to_csv('way.csv', index=False)


def save_excel(position, value, mac):
    wb = xlwt.Workbook()
    # 添加一个表
    ws = wb.add_sheet('data')
    head = [0, 'col1', 'col2', 'col3']
    for i in range(4):
        ws.write(0, i, head[i])
...
if __name__ == "__main__":
    ...
    for path_filename in path_filenames:
        ...
        way_list.append(path_data.waypoint[:, 1:3])
    ...
    fig = visualize_heatmap(heat_positions, heat_values, floor_plan_filename, width_meter, height_meter,
                            colorbar_title='mu tesla', title='Magnetic Strength', show=True)
    save_data(heat_values)
    ...
    # 聚类， 预测地址
    save_excel(heat_positions, heat_values, './ap/a' + target_wifi.replace(":", "_"))
    ...
    fig = visualize_heatmap_with_centers(heat_positions, heat_values, centers, floor_plan_filename, width_meter, height_meter,
                            colorbar_title='dBm', title=f'Wifi: {target_wifi} RSSI', show=True)
    ...
    fig = visualize_heatmap(heat_positions, heat_values, floor_plan_filename, width_meter, height_meter,
                            colorbar_title='dBm', title=f'iBeacon: {target_ibeacon} RSSI', show=True)
    ...
    fig = visualize_heatmap(heat_positions, heat_values, floor_plan_filename, width_meter, height_meter,
                            colorbar_title='number', title=f'Wifi Count', show=True)
